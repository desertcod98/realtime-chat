...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [newChat] = await db.insert(chats).values({created_at: chats.created_at.default}).returning();

    await db.insert(members).values([{chatId: newChat.id, userId: user.id}, {chatId: newChat.id, userId: invitedUser.id}]);
